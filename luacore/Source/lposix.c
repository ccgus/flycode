...
static int Pexec(lua_State *L)			/** exec(path,[args]) */
{
    ...
    for (i=1; i<n; i++) {
        [arguments addObject:[NSString stringWithUTF8String:luaL_checkstring(L, i+1)]];
    }
    
    
    NSTask *t = [NSTask launchedTaskWithLaunchPath:path arguments:arguments];
    [t waitUntilExit];
    
    int ret = [t terminationStatus];
...
{
	struct utimbuf my_times;
	time_t currtime = time(NULL);
	const char *path = luaL_checkstring(L, 1);
	my_times.modtime = luaL_optnumber(L, 2, currtime);
	my_times.actime  = luaL_optnumber(L, 3, currtime);
	return pushresult(L, utime(path, &my_times), path);
}
